...
* 修复了保存期间切换项目时，可能将新项目内容保存到原项目文件，以及错误更新当前项目路径的问题。
* 修复了部分情况下保存失败后，程序一直处于保存中状态，导致手动保存与自动保存无法继续的问题。
* 修复了连续另存为到不同目录时，音乐、背景或视频文件可能未复制到新目录的问题。
* 修复了修改 Timing Point 的时间并跨过其它 Timing Point 后，节拍线、吸附与 BPM 查询可能得到错误结果的问题。
* 修复了输入小于 0 的 BPM 时节拍线绘制可能导致程序卡住的问题。
* 修复了快速调整音符宽度后撤销可能得到错误结果的问题。
* 修复了导入或导出谱面失败后仍可能提示完成的问题。
* 修复了录制时视频解码失败后，同步取帧可能一直等待，导致程序卡死的问题。
* 修复了切换项目时节拍线绘制资源未释放，以及导入 XML / DY 谱面时额外缓冲区未释放的内存泄漏问题。
* 修复了一些其他的问题。
...
* 修复了针对部分系统的兼容性问题。
* 修复了备份不存在的项目可能导致错误的问题。
* 改进了备份项目时，因错误时间戳可能回落到 1970 年的问题。
* 现在在放置一个可能被其它 Tap 音符完全覆盖的 Tap 音符时将出现警告提示。
...
* 改进了项目文件保存流程，降低保存过程中因程序异常、系统错误或写入中断造成项目文件损坏的风险。
* 现在保存项目时会自动保留少量紧急备份。
* 修复了保存失败后仍可能显示保存成功、并把当前项目路径更新为失败目标的问题。
* 修复了打开部分包含编码错误内容的项目时，程序可能崩溃或无法正确报告错误的问题。
* 修复了部分情况下撤销/重做可能报错的问题。
* 修复了对 Timing Points 进行撤销/重做后可能得到错误结果的问题。
* 修复了保存项目时的细微卡顿问题。
...
* 更新了引擎版本。
  * 这修复了一部分情况下窗口最大化位置错误的问题。
* 修复了在少数情况下，打开项目时程序崩溃的问题。
* 修复了启动界面下，背景动画在不同帧率下表现不一致的问题。
...
- 改进了音频同步并修复了一些延迟问题。
  - 现在回放谱面时不会再有不确定的细微延迟。
  - 设备回放音乐的延迟根据文件不同相对上一版本可能有所变化。延迟设置可能因此需要重新调整。
- 改进了“禁用变调”功能的效果、延迟与修复相关的问题。
  - 移除了“禁用变调”功能的实验性标志。
- 添加了对早期版本的 XML 谱面格式的读取支持。
- 现在视频录制的默认编码器变更为 H.264 以改进兼容性。
- 为曲线生成命令新增了可用的间隔参数。
- 进一步改进了预览模式的渲染效率。
- 修复了退出全屏时窗口大小未能重置的问题。
- 修复了导入部分版本下的 dyn / xml 文件失败的问题。
- 更新了关于页面以及感谢名单。
...
* 新增了命令功能。详见 [高级功能/命令](guide/advanced-edit.md#命令)。
  * 新增了基本属性编辑命令。
  * 新增了曲线填充命令。
    * 新增了四种曲线：线性 / 余弦 / 自然三次样条 / 向心 Catmull-Rom 样条。
  * 新增了批量吸附命令。
  * 新增了批量居中命令。
  * 新增了修复出界音符的命令。
  * 新增了一些其他的高级命令。
* 修复了视频解码功能无法利用硬件加速的问题。
* 修复了视频从头播放时会出现闪烁的问题。
* 修复了开始界面颜色过渡在部分情况下失效的问题。
* 现在视频播放速度将与音乐速度保持一致。
* 现在在录制视频时，背景视频能够正常渲染了。

## [v0.2.3](https://github.com/NordLandeW/DyNode/releases/tag/v0.2.3) (2025-12-06)

该版本包含了新功能、问题修复与性能改进。

* 现在可以拖拽文件到窗口来打开项目或导入音乐、背景文件。
* 现在在没有设置 Timing 的情况下，调整拍线将会出现警告。
* 提高了程序的启动速度，并添加了预加载界面。
* 重构了视频解码功能。
  * 现在视频能够以原生分辨率渲染到屏幕上。
* 现在在初始化设置时，默认语言将跟随系统设置。
* 改善了帧时间波动导致的卡顿问题。
  * 更新 Gamemaker Runtime 版本至 2024.14。
  * 这一版本实验性地移除了对 Windows 7 的支持。
* 改善了一部分其它的动画表现。
