...
:::warning 方括号 ‘[’ 之后的内容无法显示？
由于 DyNode 使用了 Scribble 文字引擎进行渲染，方括号 `[]` 中包含的是 Scribble 可以使用的颜色标签。

如果需要正常显示方括号 `[` ，你需要使用两个左方括号 `[[` 来将方括号进行转义。例如，如果你想显示以下谱面标题 `[Praw]` ，你应该在 DyNode 输入 `[[Praw]` 来作为新的谱面标题。但是当你需要将谱面导出的时候，你可能需要额外修改导出谱面的标题。

如果你的谱面标题中包含非英文字符，则不会出现这一问题。
